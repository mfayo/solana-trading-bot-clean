...
const retrieveEnvVariable = (variableName: string, logger: Logger): string => {
...
const _rawPrivateKey = retrieveEnvVariable('PRIVATE_KEY', logger);
export const WALLET: Keypair = getWallet(_rawPrivateKey.trim());
...
export const TRAILING_TAKE_PROFIT = process.env['TRAILING_TAKE_PROFIT'] === 'true';
export const TRAILING_TAKE_PROFIT_DISTANCE = Number(process.env['TRAILING_TAKE_PROFIT_DISTANCE'] ?? '5');
